class Solution:
    def findMaximizedCapital0(self, k: int, w: int, profits: List[int], capital: List[int]) -> int:
        # maximize capital
        #
...
#        profit_sorted = 
    ...
    def findMaximizedCapital(self, k: int, w: int, profits: List[int], capital: List[int]) -> int:
        ...
        if w >= max(capital):
            min_h = []
            for i, prf in enumerate(profits):
                if i < k:
                    heapq.heappush(min_h, prf)
                    continue
                if prf > min_h[0]:
                    heapq.heappop(min_h)
                    heapq.heappush(min_h, prf)
#            print(min_h)
        ...
        for _ in range(k):
            ...
            while i < n and projects[i][0] <= cur_cap:
