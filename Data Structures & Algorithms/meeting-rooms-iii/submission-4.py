    ...
    def mostBooked(self, n: int, meetings: List[List[int]]) -> int:
        ...
        for meeting_id in range(1, len(meetings)):
            ...
            if rooms_available:
                ...
                new_t_start = earliest_end_time
                new_t_end = new_t_start + (t_end - t_start)
                new_room_id = heapq.heappop(rooms_available)
                # push
                heapq.heappush(rooms_in_use, (new_t_end, new_room_id))
                room_use_count[new_room_id] += 1
