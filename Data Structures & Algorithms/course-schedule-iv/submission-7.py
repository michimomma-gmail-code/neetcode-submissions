...
#        print(order)
    ...
    def checkIfPrerequisite(self, numCourses: int, prerequisites: List[List[int]], queries: List[List[int]]) -> List[bool]:
        ...
        print('DP version')
        res = [reachable[pre][crs] for pre, crs in queries]
        return res
...
#        print(reachable)
        ...
        def dfs(pre):
            if pre in mem:
                return mem[pre]
            reachable = set()    
            for crs in adj[pre]:# pre -> crs
                reachable.add(crs)
                reachable.update(dfs(crs))
            mem[pre] = reachable
            return reachable
        ...
        res = [crs in dfs(pre) for pre, crs in queries]
